...
# Initilize flask app
app = Flask(__name__)
config = {"CACHE_TYPE": "FileSystemCache", "CACHE_DIR": "./cache", "CACHE_THRESHOLD": 5}
...
@app.route("/api/starsforks", methods=["POST"])
...
def get_stars_forks():
    ...
    def get_stars_forks_all_repos():
        for repo_obj in all_repositories:
    ...
    get_stars_forks_all_repos()
    json_response = {}
    json_response["star_count"] = star_count
    json_response["fork_count"] = fork_count
    json_response["issue_count"] = issue_count
...
def github():
    ...
    week_created_at = df["created_at"]
    week_issue_created = pd.to_datetime(pd.Series(week_created_at), format="%Y/%m/%d")
    week_issue_created.index = week_issue_created.dt.to_period("W")
    week_issue_created = week_issue_created.groupby(level=0).size()
    week_issue_created = week_issue_created.reindex(
        pd.period_range(
            week_issue_created.index.min(), week_issue_created.index.max(), freq="W"
        ),
        fill_value=0,
    )
    ...
    month_issue_closed = month_issue_closed.reindex(
        pd.period_range(
            month_issue_closed.index.min(), month_issue_closed.index.max(), freq="m"
        ),
        fill_value=0,
    )
    ...
    week_created_at_body = {
        "issues": issues_reponse,
        "type": "week_created_at",
        "repo": repo_name.split("/")[1],
    }
